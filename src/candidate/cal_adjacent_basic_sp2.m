% 计算底层超像素相邻关系
% 顺便计算底层超像素的bbox
% adjacent_mat 超像素相邻矩阵 记录相邻像素数量
% basic_sp_boxes 超像素bbox:x_max x_min y_max y_min
function [adjacent_mat, basic_sp_boxes, sp_boundary_pixel_num, sp_pixel_num] = cal_adjacent_basic_sp2(leaves)
...
for i = 1:h   % 沿水平方向扫描
    for j = 1:w
        ...
        if j < w
            sp2 = leaves(i,j+1);
            if sp1 ~= sp2
                adjacent_mat(sp1,sp2) = adjacent_mat(sp1,sp2) + 1;
                adjacent_mat(sp2,sp1) = adjacent_mat(sp2,sp1) + 1;
            end
        end
        sp1_x = j;
        sp1_y = i;
