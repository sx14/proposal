function [net,all_line_info,new_line_frame_sp_mat] = connect_lines(net,all_line_info,long_line_info,line_frame_sp_mat,connect_line_cand_mat)
    ...
    org_line_sum = size(all_line_info,1);
    for i = 1:size(line_connect_mat,1)
        ...
        for j = 1:length(lines)
            ...
            temp_net = net(:,curr_s:curr_e,1);
            temp_net(temp_net == line) = connect_line_label;
            net(:,curr_s:curr_e,1) = temp_net;
%             for f = curr_s:curr_e
%                 frame_line = net(:,f,1);
%                 frame_line(frame_line == line) = connect_line_label;
%                 net(:,f,1) = frame_line;
%             end
            boundary_connectivity = boundary_connectivity+long_line_info(line,5);
        ...
        new_long_line_info(i,4) = end_frame - start_frame + 1;
