% connect lines with tracker
function line_connect_mat = get_connect_line_cand(sp_boxes_set,long_line_frame_sp_mat,long_line_info,resized_imgs)
line_interval_mat = get_line_interval(long_line_info);
line_connect_mat = zeros(size(line_interval_mat));
[first,second,~] = find(line_interval_mat > 0);
for i = 1:length(first)
    first_line = first(i);
    first_end_frame = long_line_info(first_line,3);
    first_line_sp = long_line_frame_sp_mat(first_line, first_end_frame);
    first_sp_boxes = sp_boxes_set{first_end_frame};
    first_sp_box = first_sp_boxes(first_line_sp,:);
    second_line = second(i);
    second_start_frame = long_line_info(second_line,2);
    second_line_sp = long_line_frame_sp_mat(second_line, second_start_frame);
