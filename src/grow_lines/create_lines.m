...
function [net,sp_boxes_set,adjacent_sp_mats,sp_boundary_connectivity_set,sp_flow_sum_set,sp_leaves_set] = create_lines(hier_set, flow_set, flow2_set,resized_imgs)
start_frame = 1;
...
for i = start_frame:end_frame
    ...
    if i == start_frame
        [net,combine_mat,sp_leaves_mat] = grow_lines(i, net, curr_hier);
    else
        last_flow = flow_set{i-1};
        last_hier = hier_set{i-1};
        curr_flow2 = flow2_set{i};
        [net,combine_mat,sp_leaves_mat] = grow_lines(i, net, curr_hier, curr_flow2, last_hier, last_flow);
    end
