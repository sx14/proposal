function avg_scores = get_cand_scores(hiers, cands, line_frame_sp_mat, cand_info, sp_flow_info_set,sp_boundary_connectivity_set)
...
for f = 1:length(hiers)
    ...
    [feats, ~] = compute_full_features(cands_hf,hier.b_feats);
    sp_flow_info = sp_flow_info_set{f};
    sp_boundary_connectivity_info = sp_boundary_connectivity_set{f};
    cands_hf(cands_hf > 0) = cands_hf(cands_hf > 0) - double(leave_sum);
    motion_cand_scores = get_motion_scores(cands_hf,sp_flow_info,sp_boundary_connectivity_info);
...
[top_k,~] = min([(cand_info(:,2)-cand_info(:,1)) long_line_ths_array],[],2);
