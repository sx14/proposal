...
function [recall, smT_IoU] = run(video_package_path,video_dir,annotation_package_path,mid_result_path,output_path,re_cal)
...
if exist(fullfile(video_path),'dir')    % validate video path
    ...
    [proposals,selected_cands,sp_leaves_set,line_frame_sp_mat] = get_proposals(video_dir,output_path,org_height,org_width,hier_set,flow_set,flow2_set,resized_imgs,re_cal);
    [result,annotations] = get_result(video_dir,annotation_path,output_path,proposals,re_cal);
    generate_masks(output_path,video_dir,result,proposals,selected_cands,sp_leaves_set,hier_set,resized_imgs,line_frame_sp_mat);
