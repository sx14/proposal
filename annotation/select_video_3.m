...
selected_result_path = '/home/sunx/output/SXD/proposals';
...
for i = 1:size(temp_annotations,1) - 2
    ...
    for j = 1:size(annotations,2)
        ...
        if ~isempty(video)
            if  video.obj_num < 3 || video.obj_num > 10
                continue;
            else
                if exist(fullfile(selected_result_path,[video.video_dir,'.mat']),'file')
                    continue;
                end
                index = index + 1;
