...
for i = 1:size(match_sp_iou,1)
    ...
    if max_iou(1) > lines(max_index(1),frame,2)                   % 找到匹配的sp并替换
        lines(max_index(1),frame,1) = lines(i,frame-1,1);      % 标上匹配的串号
        lines(max_index(1),frame,2) = max_iou(1);                 % 标上与第frame帧，第max_index个sp的iou
        lines(max_index(1),frame,3) = lines(i,frame-1,3)+1;    % 标上串当前的长度
        %         all_index = [all_index(:);bundles{max_index,frame}(:)];   % 将所有前一帧可以与当前帧当前sp连的sp记录下来
        bundles{max_index(1),frame} = max_index(1);               % 将所有可以连的当前帧sp记录下来
    end
end
...
for j = 1:size(back_match_sp_iou,1)
    ...
    if lines(j,frame,1) == 0 && max_iou(1) > 0  % 新一帧未被成功匹配的超像素，可以反向匹配
        line = lines(max_index(1),frame-1,1);
        connected_sp = find(lines(:,frame,1) == line);  % 应该只能找到1个或0个
        if size(connected_sp,1) == 0   % 正向匹配失败，直接连起来
            lines(j,frame,1) = line;
            lines(j,frame,2) = max_iou(1);
            lines(j,frame,3) = lines(max_index(1),frame-1,3) + 1;
