% 获取单个视频的处理结果
function proposals = get_proposals(video_dir, output_path, org_height, org_width, hier_set, flow_set, flow2_set,resized_imgs)
...
if ~exist(fullfile(proposal_path,proposal_file_name),'file')  % not done
    [net,hiers,adjacent_sp_mats] = connect_images(hier_set, flow_set, flow2_set, resized_imgs);      % grow sp sequences
