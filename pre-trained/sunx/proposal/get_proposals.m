...
if ~exist(fullfile(proposal_path,proposal_file_name),'file') || re_cal == true  % not done
    ...
    [net,hiers,adjacent_sp_mats,sp_boundary_connectivity_set,sp_conbine_set] = connect_images(hier_set, flow_set, flow2_set, resized_imgs);      % grow sp sequences
    [long_line_info, new_line_labels] = long_line_filter(net,sp_boundary_connectivity_set,long_line_length_ratio);        % pruning
    ...
    proposals = cands_to_proposals(hiers,cands,long_line_frame_sp_mat,cand_info,video_dir,org_height, org_width,long_line_length_ratio);
